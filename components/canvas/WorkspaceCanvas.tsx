...
export default function WorkspaceCanvas({ children, cursorEngine }: WorkspaceCanvasProps) {
  ...
  // Register cursor DOM elements with the engine
  const cursorRefCallback = useCallback(
    (agentId: string) => (el: HTMLDivElement | null) => {
      cursorEngine?.registerCursorElement(agentId, el);
    },
    [cursorEngine]
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        {agentZones.map((zone) => {
          ...
          return (
            <div
              ...
              className="absolute"
              style={{
                left: x, top: startY, width: zoneW, height: zoneH,
            ...
            >
              <div
                ...
                style={{ backgroundColor: `${zone.color}25`, border: `1px solid ${zone.color}40` }}
              >
                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: zone.color }} />
                <span className="font-mono text-[10px] font-bold tracking-[0.1em]" style={{ color: zone.color }}>
                  {zone.label}
                </span>
              </div>
            </div>
        ...
        {horizonZones.map((zone, i) => {
          ...
          return (
            <div
              ...
              className="absolute"
              style={{
                left: x, top: horizonY, width: hzW, height: horizonH,
            ...
            >
              <div
                ...
                style={{ backgroundColor: `${zone.color}20`, border: `1px solid ${zone.color}35` }}
              >
                <span className="font-mono text-[9px] font-bold tracking-[0.15em]" style={{ color: zone.color }}>
                  {zone.label}
                </span>
              </div>
            </div>
        ...
        {/* Agent cursors — DOM-ref based, NOT re-rendered by React */}
        {agents.map((agent) => (
          <div
            key={agent.id}
            ref={cursorRefCallback(agent.id)}
            className="absolute z-50 pointer-events-none"
            style={{ transform: 'translate(400px, 300px)', willChange: 'transform' }}
          >
...
export function getAgentZonePosition(agentId: string, slotIndex: number): { x: number; y: number } {
  ...
  return { x: zoneX + padding, y: zoneY + 22 + padding + row * 80 };
}
